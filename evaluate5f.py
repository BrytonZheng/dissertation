...
class Evaluate():
    ...
    def main(self, name, val):
        ...
        if val:
            ...
            valDataloader = DataLoader(t2, batch_size = args['batch_size'], shuffle = False,
                                       num_workers = args['num_worker'],
        ...
        with(t.no_grad()):
            for idx, data in enumerate(tqdm(valDataloader)):
                hist, nbrs, mask, lat_enc, lon_enc, fut, op_mask, va, nbrsva, lane, nbrslane, dis, nbrsdis, cls, nbrscls, map_positions, dsId, vehId = data
                hist = hist.to(device)
                nbrs = nbrs.to(device)
                mask = mask.to(device)
    ...
    def draw(self, hist, fut, nbrs, mask, fut_pred, train_flag, lon_man, lat_man, op_mask, indices, dsId, vehId):
        ...
        for i in range(hist.size(1)):  # 列循环
            ...
            save_path = './pic/' + str(lon_man_i + 1) + '_' + str(lat_man_i + 1) + '/' + str(self.op) + '_' + str(
                dsId[i].item()) + '-' + str(vehId[i].item()) + '.png'
            ...
            import xlwt
            wb = xlwt.Workbook()
            for i in range(9):  # 创建 9 个 sheet (适用于 `lon_man_i * 3 + lat_man_i` 的范围)
                sheet = wb.add_sheet(f"Sheet{i}")
                sheet.write(0, 0, "op")  # 添加表头
                sheet.write(0, 1, "max_y")
