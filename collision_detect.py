...
class CollisionDetect():
    ...
    def detect(self, construction_area):
        ...
        for id, v in pred.items():
            for i, pred_info in enumerate(v):
                ...
                if self.model_output_args['draw_img']:
                    if self.model_output_args['draw_pic']:
                        self.draw_collision_img(pred_info[1], id, traj, collide_idx)
    ...
    def draw_collision_img(self, frame_id, veh_id, fut_pred, collide_index):
        ...
        save_path = os.path.join(save_path, '1-' + str(veh_id) + 'collide', str(self.image_cnt) + '.png')
